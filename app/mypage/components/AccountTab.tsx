  ...
  return (
    <section className="w-full max-w-3xl">
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-bold tracking-tight text-gray-950">계정 설정</h2>
        <p className="mt-2 text-sm leading-6 text-[var(--archive-muted)]">로그인 정보와 계정 상태를 관리합니다.</p>
      </div>

      <div className="overflow-hidden rounded-lg border border-[var(--archive-line)] bg-white">
        ...
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--archive-muted)]">계정과 개인 저장 기록이 삭제됩니다.</p>
          </div>
          <button
            type="button"
            onClick={() => setShowDeleteModal(true)}
            disabled={isBusy}
            className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-red-200 bg-white px-3 text-sm font-medium text-red-600 transition hover:border-red-600 hover:bg-red-50 disabled:opacity-50"
          >
            <Trash2 className="h-4 w-4" />
            탈퇴
          </button>
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-lg border border-[var(--archive-line)] bg-white p-6 shadow-xl">
